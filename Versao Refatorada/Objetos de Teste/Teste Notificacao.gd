...
func respostaEntrada(text):
	if _plugin_singleton:
		_plugin_singleton.avisosPrint(text)
		print(text)
...
func mostraNotificacao():
	if _plugin_singleton:
		_plugin_singleton.acionarNotificacao('Tadalafina- 2 comprimi', 'Ta na hora',5,1)
	else:
		printerr("Initialization error")
...
func adicionarNotifc():
	if _plugin_singleton:
		_plugin_singleton.adicionarNotificacaoDate("Notificando na data escolhida", "testandi", 1, 22, 2, 2024, 15, 8, 0)
	else:
		print("Erro")
